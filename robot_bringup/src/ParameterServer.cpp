...
class ParameterServer : public rclcpp::Node {
  public:
    ParameterServer(
      ...
      left_leg_joint_unit_vecter_ = get_parameter(robot_name_+"_limb.limb_without_fixed_joints."+left_leg_name_+".joint_unit_vector").as_string_array();  // TODO: 多重配列は、Paramで読めない。ので、lengthは合計しちゃって良いと思われ。それか、文字列として扱うか。ほしいのは脚の始端から末端までの座標だし。それをparam_robot_description.yamlに書き込みたい。
      right_leg_joint_unit_vecter_ = get_parameter(robot_name_+"_limb.limb_without_fixed_joints."+right_leg_name_+".joint_unit_vector").as_string_array();  // TODO: 単位ベクトルは、軸方向の文字で良いのではないだろうか。それか、文字列として扱うか。
  ...
  private:
    ...
    std::vector<std::string> left_leg_joint_unit_vecter_;
    ...
    std::vector<std::string> right_leg_joint_unit_vecter_;
